...
public class GraphUI extends BaseCanvasUI {
	...
	public void init(String userId, EventHandler eventHandler) {
		addMouseListener(this);
		addMouseMotionListener(this);
		this.eventHandler = eventHandler;
		try {
			qrImage = ImageIO.read(new URL(
					"http://chart.apis.google.com/chart?cht=qr&chs=128x128&chl="
							+ URLEncoder.encode(userId) + "&chld=H|0"));
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
	...
	public void paint(Graphics graphics) {
		...
		if (qrImage != null) {
			bufferGraphics.drawImage(qrImage, 5, 5, null);
		}
		...
		for (NodeUI nodeI : lstNodes) {
			for (NodeUI nodeJ : lstNodes) {
				if (!nodeI.equals(nodeJ)) {
					...
					if (YartaWrapper.getInstance().isIncludedLink(
							nodeI.getObject(), nodeJ.getObject())) {
						drawArrow((Graphics2D) graphics, nodeI, nodeJ,
								Color.white);
					}

					if (YartaWrapper.getInstance().containsLink(
							nodeI.getObject(), nodeJ.getObject())) {
						drawArrow((Graphics2D) graphics, nodeI, nodeJ,
								Color.orange);
					}

					if (YartaWrapper.getInstance().followsLink(
							nodeI.getObject(), nodeJ.getObject())) {
						drawArrow((Graphics2D) graphics, nodeI, nodeJ,
								Color.yellow);
					}
	...
	private void drawArrow(Graphics2D graphics, NodeUI i, NodeUI j, Color color) {
