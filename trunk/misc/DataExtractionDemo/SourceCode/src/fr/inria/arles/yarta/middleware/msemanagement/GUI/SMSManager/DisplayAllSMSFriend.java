...
/**
 *An Android activity to display all SMS friend.
 *
 * <p>The  sms friends are extracted by SMSManager.extractAllSMSFriend of 
 * the msemanagement package. </p>
 * 
 * @author Nishant Kumar
 *
 */
public class DisplayAllSMSFriend extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
			...
			try{
				...
				/**
				 * The background thread that calls the start execution method of this 
				 * class.
	    		...
	    		String debug = "Inside DisplayAllSMSFriend.onCreate: ";
	    		Log.d("DisplayAllSMSFriend.onCreate",debug+ex.toString());
	 ...
	 * This function calls the SMSManager.extractAllSMSFriend of msemanagement
	...
	private boolean startExecution(){
		try{
			YLogger Log = YLoggerFactory.getLogger();
			String debug = " Inside DisplayAllSMSFriend.startExecution ";
			Log.d("DisplayAllSMSFriend.startExecution",debug);
		
			SMSManager smsManager = new SMSManager();
		
			smsInfoArrayStore = smsManager.extractAllSMSFriend(getApplicationContext());
		
			if(smsInfoArrayStore==null ||smsInfoArrayStore.size()<=0){
				Log.d("DisplayAllSMSFriend.startExecution",
						" No SMSFriend found");
			...
			if(smsInfoArrayStore!=null && smsInfoArrayStore.size()>0){
				...
				debug = "Forming ArrayAdapter for DisplayAllSMSFriend Contact ListView";
				Log.d("DisplayAllSMSFriend.startExecution",debug);
				...
				try{
					...
					debug = "ArrayAdapter cannot be formed";
					Log.d("DisplayAllSMSFriend.startExecution",debug);
					messageString = "DisplayAllSMSFriend.startExecution: ArrayAdapter was not formed";
				    return false;
				}
				
		
			}else{
				messageString = "DisplayAllSMSFriend.startExecution: "+
				                "Info extracted from SMSManager was not recognized";
			    return false;
			}
			...
			YLogger Log = YLoggerFactory.getLogger();
			String debug = "Inside DisplayAllSMSFriend.startExecution: ";
			Log.d("DisplayAllSMSFriend.startExecution",debug+ex.toString());
			messageString = debug+ex.toString();
			return false;
		}
	}
	...
	private void displayResult(){
		try{
    		...
    		if(adapter!=null){		
				...
				debug = "In DisplayResult for AllSMSFriend,Adaptor Not set ";
				Log.d("DisplayAllSMSFriend.displayResult",debug);
			    messageString = debug;
			    TextView displayContactMessage = (TextView)findViewById(R.id.displayAllContactMessage);
	     		displayContactMessage.setText(messageString);
	 ...
	 */
	  protected Dialog onCreateDialog(int position){
		  ...
		  YLogger Log = YLoggerFactory.getLogger();
			...
			Log.d("DisplayAllSMSFriend.onCreateDialog",debug);
