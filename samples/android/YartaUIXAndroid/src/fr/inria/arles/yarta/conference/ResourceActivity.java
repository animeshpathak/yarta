...
public class ResourceActivity extends BaseActivity implements View.OnClickListener,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (resource instanceof Person) {
			findViewById(R.id.personSpecificContainer).setVisibility(
					View.VISIBLE);
			findViewById(R.id.personSpecificTitle).setVisibility(View.VISIBLE);
	...
	private void onButtonPushClicked() {
		ResourcePickDialog dialog = new ResourcePickDialog(this);
		dialog.setHandler(this);
		dialog.show();
	}

	@Override
	public void onResourcesSelected(final List<String> resourceIds) {
		runner.run(new AsyncRunner.Job() {
			...
			@Override
			public void doWork() {
				String peerId = ((Person) resource).getUserId();
				total = resourceIds.size();
				for (String resId : resourceIds) {
					if (YartaWrapper.getInstance().sendPush(peerId, resId) != -1)
						sent++;
				}
			}
			...
			@Override
			public void doUI() {
				String format = getString(R.string.pick_send_percent);
				Toast.makeText(
						ResourceActivity.this,
						String.format(format, (float) sent * 100.0
								/ (float) total), Toast.LENGTH_SHORT).show();
			}
		});
	}
	...
	private void onButtonUpdateClicked() {
		runner.run(new AsyncRunner.Job() {
			...
			public void doWork() {
				...
				success = YartaWrapper.getInstance().sendUpdate(
						person.getUserId());
			}
	...
	private void runtimeGenerateUI(View view) {
		...
		for (Method method : methods) {
			...
			label.setTextAppearance(getApplicationContext(),
					R.style.AppActivityText);

			EditText edit = new EditText(this);
			edit.setWidth(200);
			edit.setSingleLine(true);
			edit.setBackgroundColor(Color.TRANSPARENT);
			edit.setTextAppearance(getApplicationContext(),
					R.style.AppActivityText);
