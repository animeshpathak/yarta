...
/**
 * An Android activity to display all sent sms friends.. 
 * 
 * <p>It display all friends extracted by checking the sms sent and contact list</p>
 * <p>The sent sms friends are extracted by SMSManager.extractSentSMSFriend of 
 * the msemanagement package. </p>
 * 
 * @author Nishant Kumar
 *
 */
public class DisplayAllSentSMSFriend extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try{
	    	...
	    	String debug = "Inside DisplayAllSentSMSFriend.onCreate: ";
	    	Log.d("DisplayAllSentSMSFriend.onCreate",debug+ex.toString());
	 ...
	 * This function calls the SMSManager.extractSentSMSFriend of msemanagement
	 ...
	 *  
	 * 
	 * @return returns true if the friend list is successfully extracted and 
	 * the array adapter
	 ...
	 */
		...
		try{
			YLogger Log = YLoggerFactory.getLogger();
			String debug = "Inside DisplaySentSMSFriend";
			Log.d("DisplayAllSentSMSFriend.startExecution",debug);

		
			SMSManager smsManager = new SMSManager();
			smsInfoArrayStore = smsManager.extractSentSMSFriend(getApplicationContext());
		
			if(smsInfoArrayStore==null ||smsInfoArrayStore.size()<=0){
				Log.d("DisplayAllSentFriend.startExecution"," No SMSFriend found");
			...
			if(smsInfoArrayStore!=null && smsInfoArrayStore.size()>0){
				...
				try{
					...
					Log.d("DisplayAllSentSMSFriend.startExecution",debug);
					messageString = "DisplayAllSentSMSFriend.startExecution: ArrayAdaptor Not formed";
					return false;
				}
			
			}
			debug = "Array Adapter not formed";
			Log.d("DisplayAllSentSMSFriend.startExecution",debug);
			messageString = "DisplayAllSentSMSFriend.startExecution: Array Adapter not Formed";
			return false;
	
		
		}catch(Exception ex){
			YLogger Log = YLoggerFactory.getLogger();
			String debug = "All SMS DataExtraction Over";
			Log.d("DisplayAllSentSMSFriend.startExecution",debug);
			messageString = "DisplayAllSentSMSFriend.startExecution: "+ex.toString();
			return false;
		...
		try{
    		...
    		if(adapter!=null){		
				...
				public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				    	...
				    	YLogger Log = YLoggerFactory.getLogger();
				      	String debug = "All Callog display Clicked at "+position;
						Log.d("DisplayAllSMSFriend.onItemClick",debug);
						...
						Log.d("DisplayAllSMSFriend.onClickListener",debug);
				...
				debug = "DisplayAllSMSFriend.displayResult,Adaptor Not set ";
				Log.d("DisplayAllSMSFriend.displayResult",debug);
			    messageString = debug;
			    TextView displayContactMessage = (TextView)findViewById(R.id.displayAllContactMessage);
	     		displayContactMessage.setText(messageString);
    		...
    		YLogger Log = YLoggerFactory.getLogger();
			String debug = " Inside DisplayAllSMSFriend.displayResult:  ";
			Log.d("DisplayAllSMSFriend.displayResult",debug+ex.toString());
		    messageString = debug+ex.toString();
		    TextView displayContactMessage = (TextView)findViewById(R.id.displayAllContactMessage);
     		displayContactMessage.setText(messageString);
            return;
	 ...
	 */
	  protected Dialog onCreateDialog(int position){
		  ...
		  YLogger Log = YLoggerFactory.getLogger();
			...
			Log.d("DisplayAllSentSMSFriend.onCreateDialog",debug);
